...
KERNCONF=GENERIC
ESPRESSOBIN_UBOOT_PORT="u-boot-marvell"
ESPRESSOBIN_UBOOT_BIN="u-boot-espresso.bin"
ESPRESSOBIN_UBOOT_PATH="/usr/local/share/u-boot/${ESPRESSOBIN_UBOOT_PORT}"
TARGET_ARCH=aarch64
...
espressobin_check_uboot ( ) {
    uboot_test $ESPRESSOBIN_UBOOT_PATH \
	    "$ESPRESSOBIN_UBOOT_PATH/Makefile" \
	    "git clone https://github.com/MarvelEmbeddedProcessors/u-boot-marvell.git /usr/local/share/u-boot"
    strategy_add $PHASE_BUILD_OTHER uboot_patch ${ESPRESSOBIN_UBOOT_PATH} \
	    `uboot_patch_files`
    strategy_add $PHASE_BUILD_OTHER uboot_configure espressobin_config
    strategy_add $PHASE_BUILD_OTHER uboot_build $ESPRESSOBIN_UBOOT_PATH
}
strategy_add $PHASE_CHECK espressobin_check_uboot
...
espressobin_install_dts_ufs ( ) {
    freebsd_install_fdt espressobin.dts boot/kernel/espressobin.dts
    freebsd_install_fdt espressobin.dts boot/kernel/espressobin.dtb
}
strategy_add $PHASE_FREEBSD_BOARD_POST_INSTALL espressobin_install_dts_ufs
